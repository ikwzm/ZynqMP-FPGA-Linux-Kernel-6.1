...
1. run scripts/build-linux-6.1.57-zynqmp-fpga-trial.sh (easy)
...
```console
shell$ git clone --depth 1 -b v6.1.57 git://git.kernel.org/pub/scm/linux/kernel/git/stable/linux-stable.git linux-6.1.57-zynqmp-fpga-trial
```

### Make Branch linux-6.1.57-zynqmp-fpga-trial

```console
shell$ cd linux-6.1.57-zynqmp-fpga-trial
shell$ git checkout -b 6.1.57-zynqmp-fpga-trial refs/tags/v6.1.57
```
...
```console
shell$ patch -p1 < ../patches/linux-6.1.57-zynqmp-fpga-lima-drv.diff
...
```console
shell$ patch -p1 < ../patches/linux-6.1.57-zynqmp-fpga-lima-clk.diff
shell$ git add --update
shell$ git commit -m "[change] clk of lima_device to use clk_bulk."
```

```console
shell$ patch -p1 < ../patches/linux-6.1.57-zynqmp-fpga-lima-irq.diff
shell$ git add --update
shell$ git commit -m "[change] lima_device to be able to specify multiple IRQ names."
```

```console
shell$ patch -p1 < ../patches/linux-6.1.57-zynqmp-fpga-drm-xlnx-align.diff
shell$ git add --all
shell$ git commit -m "[add] Dumb Buffer Alignment Size to Xilinx DRM KMS Driver for Lima support."
```

```console
shell$ patch -p1 < ../patches/linux-6.1.57-zynqmp-fpga-drm-xlnx-gem.diff
shell$ git add --all
...
```console
shell$ git tag -a 6.1.57-zynqmp-fpga-trial-1 -m "release 6.1.57-zynqmp-fpga-trial-1"
...
```console
shell$ cp arch/arm64/boot/Image.gz ../vmlinuz-6.1.57-zynqmp-fpga-trial-1
shell$ cp .config             ../files/config-6.1.57-zynqmp-fpga-trial-1
```

### Install devicetree to this repository

```console
shell$ install -d ../devicetrees/6.1.57-zynqmp-fpga-trial-1
shell$ cp arch/arm64/boot/dts/xilinx/* ../devicetrees/6.1.57-zynqmp-fpga-trial-1
```
